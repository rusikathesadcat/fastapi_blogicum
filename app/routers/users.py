...
def list_users(skip: int = 0, limit: int = 20, use_case: UserUseCase = Depends(get_user_use_case), current_user: User = Depends(get_current_active_user)):
    try:
...
def get_user(user_id: int, use_case: UserUseCase = Depends(get_user_use_case), current_user: User = Depends(get_current_active_user)):
    try:
...
def update_user(user_id: int, payload: UserUpdate, use_case: UserUseCase = Depends(get_user_use_case), current_user: User = Depends(get_current_active_user)):
    try:
...
def delete_user(user_id: int, use_case: UserUseCase = Depends(get_user_use_case), current_user: User = Depends(get_current_active_user)):
    try:
