...
def create_post(payload: PostCreate, use_case: PostUseCase = Depends(get_post_use_case), current_user: User = Depends(get_current_active_user)):
    try:
...
def update_post(post_id: int, payload: PostUpdate, use_case: PostUseCase = Depends(get_post_use_case), current_user: User = Depends(get_current_active_user)):
    try:
...
def delete_post(post_id: int, use_case: PostUseCase = Depends(get_post_use_case), current_user: User = Depends(get_current_active_user)):
    try:
