...
def create_location(payload: LocationCreate, use_case: LocationUseCase = Depends(get_location_use_case), current_user: User = Depends(get_current_active_user)):
    try:
...
def update_location(location_id: int, payload: LocationUpdate, use_case: LocationUseCase = Depends(get_location_use_case), current_user: User = Depends(get_current_active_user)):
    try:
...
def delete_location(location_id: int, use_case: LocationUseCase = Depends(get_location_use_case), current_user: User = Depends(get_current_active_user)):
    try:
